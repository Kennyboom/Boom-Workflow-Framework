...
BWF_SKILLS=(
    ...
    "bwf-context-help"
    "bwf-onboarding"
    "bwf-code-verification"  # NEW: Anti-Skip System
)
...
BWF_VERSION_FILE="$HOME/.gemini/bwf_version"
...
# Check if updating
if [ -f "$BWF_VERSION_FILE" ]; then
    OLD_VERSION=$(cat "$BWF_VERSION_FILE")
...
for skill in "${BWF_SKILLS[@]}"; do
    skill_dir="$SKILLS_DIR/$skill"
    mkdir -p "$skill_dir"
    if curl -fsSL "$REPO_BASE/bwf_skills/$skill/SKILL.md" -o "$skill_dir/SKILL.md" 2>/dev/null; then
...
# 5. Save version
mkdir -p "$HOME/.gemini"
echo "$CURRENT_VERSION" > "$BWF_VERSION_FILE"
...
if [ ! -f "$GEMINI_MD" ]; then
    echo "$BWF_INSTRUCTIONS" > "$GEMINI_MD"
    echo -e "${GREEN}✅ Đã tạo Global Rules (GEMINI.md)${NC}"
else
    # Remove old BWF section and append new one
    BWF_MARKER="# BWF - Antigravity Workflow Framework"
    if grep -q "$BWF_MARKER" "$GEMINI_MD" 2>/dev/null; then
        # Remove from marker to end of file
        sed -i.bak "/$BWF_MARKER/,\$d" "$GEMINI_MD" 2>/dev/null || sed -i '' "/$BWF_MARKER/,\$d" "$GEMINI_MD"
        rm -f "$GEMINI_MD.bak"
    fi
    echo "$BWF_INSTRUCTIONS" >> "$GEMINI_MD"
...
echo ""
echo -e "${CYAN}👉 Bạn có thể dùng BWF ở BẤT KỲ project nào ngay lập tức!${NC}"
echo -e "👉 Thử gõ '/plan' để kiểm tra."
echo -e "👉 Kiểm tra update: '/bwf-update'"
echo ""
