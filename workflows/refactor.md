...
⚠️ GATE: KHÔNG TIẾN HÀNH nếu chưa hoàn thành 5/5 bước trên."
```
...
Nếu refactoring có thể ẢNH HƯỞNG performance:
→ Benchmark TRƯỚC và SAU
→ Ví dụ: Extract Method có thể thêm function call overhead
→ Nếu performance giảm > 5% → Cân nhắc lại
```
...
```
1. ĐO TRƯỚC, MỔ SAU — Không bao giờ refactor mà chưa đo metrics
2. LOGIC KHÔNG ĐỔI — Refactor chỉ thay CẤU TRÚC, không thay HÀNH VI
3. MICRO-STEPS — 1 refactoring = 1 commit, test sau mỗi bước
4. FOWLER'S CATALOG — Dùng TÊN CHUẨN cho techniques (Extract Method, v.v.)
5. BEFORE/AFTER — Luôn so sánh metrics trước và sau
6. SAFETY FIRST — Backup, test baseline, rollback plan TRƯỚC KHI bắt đầu
7. SOLID AUDIT — Kiểm tra 5 nguyên tắc SOLID BẮT BUỘC
8. DEBT QUANTIFICATION — Lượng hóa nợ kỹ thuật bằng SỐ (giờ, %, rating)
9. CHỨNG MINH ROI — Stakeholder cần thấy VALUE, không chỉ "code sạch hơn"
```
